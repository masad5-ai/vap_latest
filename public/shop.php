...
<section class="section">
    ...
    <div class="grid grid-3">
        <?php foreach ($products as $product): ?>
            <?php $effective = product_price($product); $onSale = isset($product['sale_price']) && $product['sale_price'] < $product['price']; ?>
            <article class="card elevated">
                <img src="<?= htmlspecialchars($product['image']) ?>" alt="<?= htmlspecialchars($product['name']) ?>">
                <div class="card-body">
                    <div class="pill-row">
                        <span class="pill"><?= htmlspecialchars($product['category']) ?></span>
                        <?php if (!empty($product['promo'])): ?><span class="pill muted"><?= htmlspecialchars($product['promo']) ?></span><?php endif; ?>
                    ...
                    <div class="price-row">
                        <div>
                            <strong>$<?= number_format($effective, 2) ?></strong>
                            <?php if ($onSale): ?><span class="muted" style="text-decoration:line-through; margin-left:6px;">$<?= number_format($product['price'], 2) ?></span><?php endif; ?>
                        </div>
