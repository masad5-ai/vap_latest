        ...
        switch ($action) {
            ...
            case 'update_cart':
                ...
                if (!empty($_POST['shipping_method'])) {
                    set_shipping_method($_POST['shipping_method']);
                }
                $message = 'Cart updated';
                $section = 'cart';
                break;
            case 'select_shipping':
                $selectedMethod = set_shipping_method($_POST['shipping_method'] ?? '');
            ...
            case 'checkout':
                ...
                $shippingMethod = set_shipping_method($_POST['shipping_method'] ?? selected_shipping_method());
            ...
            <section class="panel">
                ...
                <?php else: ?>
                    ...
                    <div class="totals">
                        ...
                        <?php $shippingDetails = $totals['shipping_details'] ?? null; ?>
                        <p>Shipping: $<?= number_format($totals['shipping'], 2) ?>
                            <?php if ($shippingDetails): ?>
                                <span class="pill muted"><?= htmlspecialchars($shippingDetails['label']) ?><?= $totals['shipping'] <= 0 ? ' • Free' : '' ?></span>
                            <?php endif; ?>
                        </p>
                    ...
                    <?php if (!empty($shippingOptions)): ?>
                        <form method="post" class="shipping-chooser">
                            <input type="hidden" name="action" value="select_shipping">
                            <label>Shipping option
                                <select name="shipping_method">
                                    <?php foreach ($shippingOptions as $key => $option): ?>
                                        <option value="<?= htmlspecialchars($key) ?>" <?= ($totals['shipping_details']['key'] ?? selected_shipping_method()) === $key ? 'selected' : '' ?>>
                                            <?= htmlspecialchars($option['label']) ?> — <?= htmlspecialchars($option['eta'] ?? '') ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </label>
                            <button class="button ghost" type="submit">Update shipping</button>
                        </form>
                    <?php endif; ?>
        ...
        <?php if ($section === 'checkout'): ?>
            <section class="panel">
                ...
                <form method="post" class="form-grid">
                    ...
                    <?php if (!empty($shippingOptions)): ?>
                        <fieldset class="fieldset span-2">
                            <legend>Shipping method</legend>
                            <?php foreach ($shippingOptions as $key => $option): ?>
                                <label class="radio-row">
                                    <input type="radio" name="shipping_method" value="<?= htmlspecialchars($key) ?>" <?= ($totals['shipping_details']['key'] ?? selected_shipping_method()) === $key ? 'checked' : '' ?>>
                                    <div>
                                        <strong><?= htmlspecialchars($option['label']) ?></strong>
                                        <p class="muted small">
                                            <?= htmlspecialchars($option['eta'] ?? '') ?> •
                                            <?= ($totals['subtotal'] ?? 0) >= ($option['free_over'] ?? 0) && ($option['free_over'] ?? 0) > 0 ? 'Free over $' . number_format($option['free_over'], 0) : '$' . number_format($option['base_rate'] ?? 0, 2) ?>
                                        </p>
                                    </div>
                                </label>
                            <?php endforeach; ?>
                            ...
                            <div class="timeline">
                                <?php foreach ($userOrders as $order): ?>
                                    <div class="timeline-item">
                                        ...
                                        <div class="timeline-meta">#<?= $order['id'] ?> • <?= htmlspecialchars($order['status']) ?> • <?= htmlspecialchars($order['payment_method'] ?? 'custom_gateway') ?><?= $shippingLabel ? ' • ' . htmlspecialchars($shippingLabel) : '' ?></div>
